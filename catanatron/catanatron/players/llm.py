...
class LLMPlayer(Player):
    ...
    def _format_game_state_for_llm(self, game, playable_actions) -> str:
        ...
        try:
            ...
            for i, action in enumerate(playable_actions):
                ...
                prompt_lines.append(f"  {i}: {action.action_type.name} - {action_desc}{priority}")

            prompt_lines.append("\nChoose action number (respond with only the number):")
    ...
    def _format_single_action(self, action) -> str:
        ...
        try:
            if action.action_type.name == "BUILD_SETTLEMENT":
                return f"Node {action.value}"
            elif action.action_type.name == "BUILD_CITY":
                return f"Node {action.value}"
            elif action.action_type.name == "BUILD_ROAD":
                return f"Nodes {action.value}"
            elif action.action_type.name == "BUY_DEVELOPMENT_CARD":
                return "Purchase dev card"
            elif action.action_type.name == "MOVE_ROBBER":
                if hasattr(action.value, '__len__') and len(action.value) >= 2:
                    coord, victim = action.value[0], action.value[1]
                    victim_str = victim.value if hasattr(victim, 'value') else str(victim) if victim else "None"
                    return f"To {coord}, steal from {victim_str}"
                return f"Move robber"
            elif action.action_type.name == "MARITIME_TRADE":
                if hasattr(action.value, '__len__') and len(action.value) >= 2:
                    give, get = action.value[0], action.value[1]
                    return f"Give {RESOURCES[give]} get {RESOURCES[get]}"
                return "Port trade"
            elif action.action_type.name == "OFFER_TRADE":
                return "Propose trade"
            else:
                return action.action_type.name.lower()
        except Exception:
            return action.action_type.name.lower()
    ...
    def _parse_llm_response(self, response_text: str, playable_actions: list[Action]) -> Action | None:
        ...
        try:
            ...
            # 如果直接解析失败，使用正则表达式提取数字
            numbers = re.findall(r'\d+', response_text)
            if numbers:
                chosen_index = int(numbers[-1])  # 取最后一个数字
                if 0 <= chosen_index < len(playable_actions):
                    print(f"LLM {self.color.value}: Extracted number {chosen_index} from response: '{response_text}'")
                    return playable_actions[chosen_index]
                else:
                    print(
                ...
                print(
                    f"LLM response contains no numbers: '{response_text}'. "
                    f"Defaulting to random valid action."
                )
                return random.choice(playable_actions) if playable_actions else None
                ...
                response = self.client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        thinking_config=types.ThinkingConfig(
                            thinking_budget=1024,  # 中等思考預算，適合複雜決策
    ...
    def _system_prompt(self):
        return """# CATAN STRATEGY SYSTEM

You are an expert Catan AI player. Use these core principles:

## VICTORY CONDITIONS
Goal: 10 victory points
Points from: Settlements (+1), Cities (+2), Longest Road (+2), Largest Army (+2), VP Cards (+1)

## BUILDING COSTS
Settlement: 1 Wood + 1 Brick + 1 Sheep + 1 Wheat
City: 2 Wheat + 3 Ore (upgrade existing settlement)
Road: 1 Wood + 1 Brick
Dev Card: 1 Sheep + 1 Wheat + 1 Ore

## CORE STRATEGY HIERARCHY

### PRIORITY 1: BUILD SETTLEMENTS
- Settlements are your primary scoring method
- Target: 4-5 settlements minimum
- Choose high-probability numbers (6,8,5,9)
- Ensure resource diversity

### PRIORITY 2: STRATEGIC CITIES
- Only after securing 3+ settlements
- Upgrade highest-producing settlements
- Cities double resource output

### PRIORITY 3: TACTICAL ROADS
- Roads enable settlement placement only
- Never build roads without settlement plan
- Consider longest road bonus only if achievable

### PRIORITY 4: DEVELOPMENT CARDS
- Buy when unable to build structures
- Use Knights for robber control
- Save VP cards for surprise victory

## CRITICAL NUMBERS
Best: 6,8 (5/36 probability each)
Good: 5,9 (4/36 probability each)
Avoid: 2,12 (1/36 probability each)

## TRADING RULES
- Trade only to enable building
- Never help leading player
- Use ports efficiently (3:1 general, 2:1 specific)

## ROBBER STRATEGY
- Target leading player's best tiles
- Steal from players with most cards
- Protect your own key resources
...
5. Should I trade or use robber?

## ACTION PRIORITY
1. Build settlement if possible
2. Build city if 3+ settlements
3. Build road toward settlement site
4. Trade for building resources
5. Buy dev cards if excess resources

CRITICAL: Settlements win games, not roads or resources. Build aggressively."""
